...
async function main(): Promise<void> {
  ...
  const approveTx = await usdc.approve(cUsdcProtectionMarketAddr, MaxUint256);
  await approveTx.wait();

  // Let's verify this approve transaction was successful
  const allowance = await usdc.allowance(signer.address, cUsdcProtectionMarketAddr);
  ...
  const cUsdcProtectionMarket = new Contract(cUsdcProtectionMarketAddr, cozyTokenAbi, signer);
  const mintTx = await cUsdcProtectionMarket.mint(parsedSupplyAmount);
  const { log: mintLog, receipt: mintReceipt } = await findLog(mintTx, cUsdcProtectionMarket, 'Mint', provider);
  const cUsdcProtectionMarketSymbol = await cUsdcProtectionMarket.symbol();
  const cUsdcProtectionMarketName = await cUsdcProtectionMarket.name();
  logSuccess(
    `${cUsdcProtectionMarketName} (${cUsdcProtectionMarketSymbol}) successfully minted in transaction ${mintReceipt.transactionHash}`
  );
}
